...
// const img = [{   md5:"b797f86724abaca3b0542fb963e8679a",   name:"file.png",
// status:"done",   uid:"rc-upload-1492242576856-2",
...
class CaseEditModal extends Component {
  ...
  componentWillReceiveProps(nextprops) {
    ...
    if (nextprops.cases.images && nextprops.cases.images.length > 0) {
      ...
      this.setState({addBefore, addAfter})
    }
        ...
        if (f.url) {
          ...
          this
            ...
            .push({
              name: f.name,
              uid: f.uid,
              url: '/api/file/' + f.response.md5list[0],
              status: f.status,
              md5: f.response.md5list[0]
            });
        }
      }
    })
    this.setState({fileList})
    // addAfter:this.props.case.images ?  this.props.case.images.map(v =>
    // v.case_type == "after" ?  v : [])  : [], addBefore : this.props.case.images ?
    // this.props.case.images.map(v => v.case_type == "before" ?  v : [])  : [],
  }
  addAfterImgHandler = ({fileList}) => {
    console.log(fileList);
    this.setState({addAfter: fileList})
  }
  ...
  getIcons() {
    var icons = [
      "source | undo redo | bold italic underline strikethrough fontborder emphasis | ", "paragraph fontfamily fontsize | superscript subscript | ", "forecolor backcolor | removeformat | insertorderedlist insertunorderedlist | sel" +
          "ectall | ",
      "cleardoc  | indent outdent | justifyleft justifycenter justifyright | touppercas" +
          "e tolowercase | ",
    ...
    return {
      ...
      qiniu: {
        ...
        genKey: function (options) {
          return options.file.type + "-" + options.file.size + "-" + options
            .file
            .lastModifiedDate
            .valueOf() + "-" + new Date().valueOf() + "-" + options.file.name;
        }
      }
    }
  }
  // clickImgHandler = (e) => {   console.log(e.target); }
  ...
  searchHandler = async(value) => {
    ...
    let categorys = await service.getDataService("Category", {
      "_id": {
        "$in": list[0].category_num
      }
    });
  ...
  render() {
    ...
    for (var i = 0; i < count; i++) {
      editors.push({icons: icons, plugins: plugins})
    }
    ...
    const afterAdd = (
      <div
        ...
        {this.state.addAfter.length > 0
          ? <img
              id="addAffter"
              src={this.state.addAfter[0].url}
              style={{
              width: "200px",
              height: "200px"
            }}/>
    ...
    const beforeAdd = (
      <div
        ...
        {this.state.addBefore.length > 0
          ? <img
              id="addBefore"
              src={this.state.addBefore[0].url}
              style={{
              width: "200px",
              height: "200px"
            }}/>
        ...
        <Modal
          ...
          <Form horizontal onSubmit={this.okHandler}>
            <Row justify="space-between">
              <Col span={12}>
                <FormItem className={styles.FormItem} {...formItemLayout} label="URL">
                  {getFieldDecorator('url', {
                    ...
                    initialValue: cases.url
                  })(<Input size="small"/>)}</FormItem>
                <FormItem className={styles.FormItem} {...formItemLayout} label="项目名称">
                  {getFieldDecorator('project_name', {
                    ...
                    initialValue: cases.project_name
                  })(<Input size="small"/>)}</FormItem>
              ...
              <Col span={12}>
                <FormItem className={styles.FormItem} {...formItemLayout} label="请选择案例户型">
                ...
                <FormItem className={styles.FormItem} {...formItemLayout} label="请选择案例空间">
                  {getFieldDecorator('space_node', {
                    rules: [
                      {
                        required: true,
                        message: '请选择案例空间!'
                ...
                <FormItem className={styles.FormItem} {...formItemLayout} label="请选择案例风格">
                ...
                <Row>
                  <p>
                    <b>添加商品</b>(请安住商品拖动到左侧案例图上对应商品为止)</p>
                </Row>
                ...
                <Row className={styles.Row}>
                  <Table columns={columnIn} dataSource={this.state.points} pagination={false}/>
                </Row>
              </Col>
            </Row>
            <Row className={styles.Row}>
