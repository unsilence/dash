...
class CaseManagesHistoryComponent extends Component {
  constructor(props) {
    super(props);
    this.state={
  ...
  deleteHandler = (itm) => {
    this.state.dispatch({
      type: 'casemanages/historyrecoms/remove',
      payload: {id:itm._id},
    });
  }
...
function mapStateToProps (state){
  const {list, total, page ,categoryMap} = state["casemanages/historyrecoms"];
  return {
    loading: state.loading.models["casemanages/historyrecoms"],
    historyList : list.historyList,
    historyCount : total ? total.historyCount : 0,
    page,
    categoryMap
  };
}
...
export default connect(mapStateToProps,mapDispatchToProps)(CaseManagesHistoryComponent);
