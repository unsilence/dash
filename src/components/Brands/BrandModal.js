...
class BrandEditModal extends Component {
  ...
  render() {
    ...
    const { _id,brandId,name,name_en,shortcut,firstletter,note} = this.props.record;
    ...
    return (
      <span>
        ...
        <Modal
          title={_id ? "修改："+brandId : '新建'}
          visible={this.state.visible}
          onOk={this.okHandler}
          onCancel={this.hideModelHandler}
        >
          <Form horizontal onSubmit={this.okHandler}>
          <FormItem className={styles.FormItem} {...formItemLayout} label="品牌ID" >    {getFieldDecorator('brandId',             {initialValue: brandId})(<Input size="small" />)  }</FormItem>
