  ...
  function getParent(item, elems) {
    elems.forEach(v => {
      if (v._id === item.serial_num) {
        v.children ? v.children.push({ "value": item._id, "label": item.name, "cnum": item.cnum, 'key': item._id }) : (v.children = [{ "value": item._id, "label": item.name, "cnum": item.cnum, 'key': item._id }]);
      }
    })
  };
