...
class HotProductsHistoryComponent extends Component {
  constructor(props) {
    super(props);
    this.state={
  ...
  deleteHandler = (itm) => {
    this.state.dispatch({
      type: 'hotproducts/historyrecoms/remove',
      payload: {id:itm._id},
    });
  }
...
function mapStateToProps (state){
  const {list, total, page ,categoryMap} = state["hotproducts/historyrecoms"];
  return {
    loading: state.loading.models["hotproducts/historyrecoms"],
...
export default connect(mapStateToProps,mapDispatchToProps)(HotProductsHistoryComponent);
