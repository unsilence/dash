...
export var recommendOption = function (banner) {

    banner.effects.fetch = function* ({ payload: { page } }, { call, put }) {
        // 无条件的
        const recommends = yield call(service["fetchRecommendPage"], 'Recommend', { page, rtype: '2' });
        const rd = { data: recommends.data.data.list, total: recommends.data.data.count, page: parseInt(page) }
        console.log(rd);
