...
export var editOrderOption = function (orders) {
    ...
    orders.effects.fetch = function* ({ payload: { page } }, { call, put }) {
        ...
        let projectList = yield call(service["getDataService"], "Address", { "cnum": { "$in": projectIns }, "valid": true });  // 得到order列表里显示的address属性
        ...
        const addressList = yield call(service["getDataService"], "Address", { "cnum": { "$in": projectIds } });  // 余sku相关的所有工程

        const skuInSkuList = yield call(service["getDataService"], 'Sku', { 'cnum': { '$in': skuIds } });   // 请求回sku表里面和order 里 sku_num 相同的数据
        ...
        _skus.forEach(v => {
            v.attributes.forEach(k => {
                k.colorProps = [];
                if (k.dtype === "array") {
                    Array.isArray(JSON.parse(k.value.replace(/\\/g, "")))
